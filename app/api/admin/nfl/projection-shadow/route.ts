...
export async function GET(request: NextRequest) {
  ...
  const modelVersion = request.nextUrl.searchParams.get("modelVersion")?.trim() || NFL_PROJECTION_V2;
  if (!Number.isInteger(season) || season < 2000 || season > 2100) return NextResponse.json({ error: "Valid NFL season required." }, { status: 400 });
  if (modelVersion.length > 160) return NextResponse.json({ error: "Invalid NFL projection model version." }, { status: 400 });
  ...
  const [cache, players, slate] = await Promise.all([
    supabaseAdmin.from("nfl_projection_stat_cache").select("provider_player_id,local_player_id,position,season,model_version,as_of,generated_at,projected_stats,confidence,sample,components,source_latest_game_at,source_latest_known_at").eq("season", season).eq("model_version", modelVersion),
  ...
  const rows = eligible.map(player => {
    ...
    if (!row) return { playerId: player.id, playerName: player.name, position: player.position, availability: "unavailable", reason: "zero_current_or_prior_season_history" };
  ...
  return NextResponse.json({ season, slateId: slate.data?.id ?? null, slateRulesSnapshot: slate.data?.rules_snapshot ?? null, modelVersion,
