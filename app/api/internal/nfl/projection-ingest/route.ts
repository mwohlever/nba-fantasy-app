...
import { auditNflObservationSeason, loadActiveNflObservationPlayers, existingNflObservationHashes, insertNflObservationVersions } from "@/lib/analytics/nfl/observationRepository.server";
...
/** Machine-to-machine factual ingestion only. It never generates NFL projections. */
export async function POST(request: Request) {
  ...
  try {
    ...
    if (body.action !== "observations" || !Array.isArray(body.observations) || body.observations.length === 0 || body.observations.length > 500) {
      return NextResponse.json({ error: "Use action eligible, audit, or 1–500 normalized factual observations." }, { status: 400, headers: { "Cache-Control": "no-store" } });
    }
