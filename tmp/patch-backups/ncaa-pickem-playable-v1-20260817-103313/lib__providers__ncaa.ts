...
function mapTeam(
  ...
  return {
    ...
    /*
     * ESPN scoreboard competitor objects expose
     * the poll rank under curatedRank.current.
     *
     * Anything outside 1-25 is intentionally
     * treated as unranked for this mini-game.
     */
    rank:
      positiveRank(
...
export async function fetchNcaaPickEmWeek({
  ...
  const params =
    new URLSearchParams({
      ...
      /*
       * FBS grouping. Keeping this explicit prevents
       * lower-division duplicate/event noise.
       */
      groups: "80",
    });
