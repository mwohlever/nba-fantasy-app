...
for(const count of [1,2,3,4])test(`${count*18}-hole confirmed event maps only existing rounds`,()=>{
 const w=weekend({...ready,startedRounds:[1],regulationRoundCount:count});assert.deepEqual(w.regulationRounds,[3,4].filter(r=>r<=count));assert.equal(w.state,count<=2?'unavailable':'open');
});
test('shortening changes round availability, never invents R4',()=>{
 assert.deepEqual(weekend({...ready,startedRounds:[1],regulationRoundCount:3}).regulationRounds,[3]);
 assert.deepEqual(weekend({...ready,startedRounds:[1],regulationRoundCount:null}).regulationRounds,[]);
 assert.equal(weekend({...ready,startedRounds:[1],regulationRoundCount:null}).state,'open');
});
...
for(const eligibility of ['made_cut','continuing','missed_cut','withdrawn','disqualified','did_not_start','unknown'])test(`eligibility ${eligibility} is explicit`,()=>{
 const w=weekend({...ready,startedRounds:[1],players:[{playerId:1,eligibility}]});
 ...
 assert.deepEqual(w[key],[1]);if(eligibility==='unknown')assert.equal(w.canBuildRoster,true);
});
test('accepted terminal status overrides stale made-cut classification',()=>{
 for(const status of ['cut','withdrawn','disqualified','did_not_start'])assert.deepEqual(weekend({...ready,startedRounds:[1],acceptedEvents:[{player_id:1,status}]}).ineligiblePlayerIds,[1]);
});
test('incomplete/empty field and unknown cut never close an R1-open Weekend window',()=>{
 for(const patch of [{fieldComplete:false},{players:[]},{cut:'unknown'},{round2Complete:null},{round3NotStarted:null}])assert.equal(weekend({...ready,startedRounds:[1],...patch}).state,'open');
 assert.equal(weekend({...ready,tournamentComplete:null}).state,'unavailable');
});
...
test('tee time and current-round hint alone do not establish an R3 lock',()=>assert.equal(weekend({...ready,startedRounds:[1],acceptedEvents:[{player_id:1,current_round:3,tee_time:'2026-09-12T12:00:00Z',golf_rounds:[{round_number:3,holes_completed:0}]}]}).state,'open'));
