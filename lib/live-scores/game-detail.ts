...
async function loadPreviewTeam(
  ...
  const coreUrl =
    `${ESPN_FOOTBALL_CORE_BASE}/seasons/${seasonYear}/types/${seasonType}` +
    `/teams/${teamId}/statistics`;

  const siteUrl =
    `${ESPN_FOOTBALL_BASE}/teams/${teamId}/statistics?season=${seasonYear}`;
  ...
  try {
    ...
    console.warn(`Failed to load football preview stats for team ${teamId}`, error);
    return null;
  }
}
...
async function loadSeasonPlayerStats(
  ...
  try {
    const rosterResponse = await fetch(
      `${ESPN_FOOTBALL_BASE}/teams/${teamId}/roster?season=${seasonYear}`,
      {
        cache: "no-store",
        headers: { Accept: "application/json" },
      },
    ...
    console.warn(
      `Failed to load football season player stats for team ${teamId}`,
      error,
    );
    return null;
  }
...
return async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(
      `${ESPN_FOOTBALL_BASE}/summary?event=${id}`,
      {
        cache: "no-store",
        headers: {
    ...
    if (!response.ok) {
      throw new Error(
        `ESPN football game summary failed: ${response.status}`,
      );
    }
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "Unable to load football game detail.",
      },
      { status: 500 },
    );
  }
