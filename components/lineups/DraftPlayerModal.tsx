...
export default function DraftPlayerModal({
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 px-3 py-4 sm:px-4"
      onClick={() => setDraftingPlayer(null)}
    >
      <div
        className="w-full max-w-2xl max-h-[85vh] overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl"
                      ...
                      <div className="min-w-0">
                        ...
                        item.turnovers !== 0 ? (
                          <div className="mt-0.5 text-xs leading-tight text-slate-500">
                            PTS {item.points} • REB {item.rebounds} • AST{" "}
                            {item.assists} • STL {item.steals} • BLK{" "}
                            {item.blocks} • TO {item.turnovers}
                          </div>
                        ) : null}
                      </div>
