...
type GameDetailResponse = {
  ...
  gameStory?: import("@/lib/live-scores/metadata").GameStory | null;
...
function totalRecord(competitor?: EspnCompetitor) {
  return (
    ...
    competitor?.record?.find((record) => record.type === "total")?.displayValue ||
    competitor?.record?.[0]?.summary ||
    competitor?.record?.[0]?.displayValue ||
    ""
  );
}
...
function PlayerDetailModal({
    ...
    async function load() {
      ...
      try {
        const response = await fetch(
          `${apiBase}/player-detail?athleteId=${encodeURIComponent(
            player.id,
  ...
  }, [player.id, player.teamId, season, apiBase]);
...
export default function GameCenterModal({
  ...
  const loadGameDetail = useCallback(
    async ({ initial = false }: { initial?: boolean } = {}) => {
      if (!initial && requestRef.current) return;
  ...
  const playerTeams = useMemo(() => {
    if ((competition?.status?.type?.state ?? game.status) !== "pre") {
  ...
  }, [game.status, competition?.status?.type?.state, detail, away, home]);
  ...
  return (
    <div
      className="fixed inset-0 z-[10000] bg-slate-950/55 p-0 sm:p-3"
      onClick={onClose}
    >
      <div
      ...
      >
          ...
          <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3">
            <div className="min-w-0 text-center">
              ...
              <div className="mt-1 truncate text-xs font-black text-slate-900">
                {isLive && detail?.possessionTeamId === String(away?.id) ? <span role="img" aria-label="Possession" className="mr-1 text-[11px]">🏈</span> : null}
                {away?.rank ? `#${away.rank} ` : ""}
            ...
            <div className="min-w-[92px] text-center">
              <div className="text-[11px] font-bold uppercase tracking-wide text-slate-500">
                {statusText}
                {(detail?.broadcast ?? game.broadcast)?.network ? <span className="block normal-case tracking-normal">{(detail?.broadcast ?? game.broadcast)?.network}</span> : null}
              </div>

              {(competition?.status?.type?.state ?? game.status) === "pre" ? (
            ...
            <div className="min-w-0 text-center">
              ...
              <div className="mt-1 truncate text-xs font-black text-slate-900">
                {isLive && detail?.possessionTeamId === String(home?.id) ? <span role="img" aria-label="Possession" className="mr-1 text-[11px]">🏈</span> : null}
                {home?.rank ? `#${home.rank} ` : ""}
        ...
        <div className="grid grid-cols-3 border-b border-slate-200">
          {(["summary", "pbp", "stats"] as const).map((value) => (
            <button
              key={value}
              type="button"
              onClick={() => setTab(value)}
              className={`px-1 py-2.5 text-xs font-bold whitespace-nowrap ${
                tab === value
                  ? "border-b-2 border-sky-500 text-sky-600"
                  : "text-slate-500"
              }`}
            >
              {value === "summary" ? "Summary" : value === "pbp" ? "Play-by-Play" : "Player Stats"}
            </button>
          ))}
        </div>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto pb-24 sm:pb-6">
          ...
          {loading ? (
            ...
            <div className="space-y-5 p-4">
              {competition?.status?.type?.state === "post" && detail?.gameStory ? (
                <section>
                  <h2 className="mb-2 text-xs font-black uppercase tracking-wider text-slate-500">Game Story</h2>
                  <h3 className="text-base font-bold leading-snug">{detail.gameStory.headline}</h3>
                  {detail.gameStory.description ? <p className="mt-2 text-sm leading-relaxed text-slate-600">{detail.gameStory.description}</p> : null}
                  <div className="mt-2 flex items-center gap-3 text-xs text-slate-500">
                    <span>{detail.gameStory.source || "ESPN"}</span>
                    {detail.gameStory.url ? <a href={detail.gameStory.url} target="_blank" rel="noopener noreferrer" className="font-bold text-sky-600">Read more on ESPN ↗</a> : null}
                  </div>
                </section>
              ) : null}
              {(competition?.status?.type?.state ?? game.status) === "pre" ? (
                ...
                <>
                  ...
                  {detail?.scoringPlays?.length ? (
                    <section>
                      <h2 className="mb-2 text-xs font-black uppercase tracking-wider text-slate-500">Scoring Summary</h2>
                      <div className="divide-y divide-slate-100">
                        {detail.scoringPlays.map((play, index) => (
                          <div key={play.id || index} className="py-2 text-sm">
                            <div className="flex justify-between gap-2 text-xs font-bold text-slate-500">
                              <span>{play.period?.number ? quarterLabel(play.period.number) : ""}{play.clock?.displayValue ? ` · ${play.clock.displayValue}` : ""}</span>
                              {play.awayScore != null && play.homeScore != null ? <span>{play.awayScore} - {play.homeScore}</span> : null}
                            </div>
                            <p className="mt-1 text-slate-700">{play.text}</p>
                          </div>
                        ))}
                      </div>
                    </section>
      ...
      {selectedPlayer ? (
        <PlayerDetailModal
          key={`${selectedPlayer.teamId}:${selectedPlayer.id}`}
          apiBase={apiBase}
          player={selectedPlayer}
          season={
            detail?.seasonYear ||
