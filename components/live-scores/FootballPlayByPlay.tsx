...
import { isMeaningfulFootballPlay, nextFootballResultingState, normalizeFootballVisualizationPlay, footballPlayId, withFootballResultingState, type FootballFieldTeam, type FootballRawPlay } from "@/lib/live-scores/footballPlayVisualization";

type Drive = { plays?: FootballRawPlay[] };

export default function FootballPlayByPlay({ drives, isLive, offenseNames, homeTeam, awayTeam, initialPeriod }: { drives: Drive[]; isLive: boolean; offenseNames: Record<string, string>; homeTeam?: FootballFieldTeam; awayTeam?: FootballFieldTeam; initialPeriod?: number | null }) {
  const grouped = useMemo(() => footballPlaysByQuarter(drives), [drives]);
  const all = useMemo(() => grouped.flatMap(group => group.plays), [grouped]);
  const chronological = useMemo(() => [...all].reverse(), [all]);
  ...
  const selectedBase = selectedRaw ? normalizeFootballVisualizationPlay(selectedRaw, 0, { offenseAbbreviation: selectedRaw.start?.team?.id ? offenseNames[String(selectedRaw.start.team.id)] : null, teamAbbreviations: Object.values(offenseNames), homeAbbreviation: homeTeam?.abbreviation, awayAbbreviation: awayTeam?.abbreviation, homeTeamId: homeTeam?.id, awayTeamId: awayTeam?.id }) : null;
  const resultingState = useMemo(() => {
    if (!selectedRaw || !selectedBase?.semanticLabel || selectedBase.administrativeLabel) return null;
    return nextFootballResultingState(chronological, footballPlayId(selectedRaw), { teamAbbreviations: Object.values(offenseNames) });
  }, [chronological, offenseNames, selectedBase?.administrativeLabel, selectedBase?.semanticLabel, selectedRaw]);
  const selectedWithState = selectedBase ? withFootballResultingState(selectedBase, resultingState, resultingState ? offenseNames[resultingState.teamId] : null) : null;
  // normalizeFootballVisualizationPlay already produces stable display-space coordinates.
  // Do not mirror this selected-play object again before it reaches the field.
  const selected = selectedWithState;
