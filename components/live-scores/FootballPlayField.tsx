...
function Replay({ play, compact, enabled, reducedMotion, direction }: { play: FootballVisualizationPlay; compact: boolean; enabled: boolean; reducedMotion: boolean; direction: "left" | "right" | null }) {
  ...
  if (play.renderMode === "semantic") {
    ...
    const compactOutcome = compact && Boolean(play.semanticSecondaryLabel);
    return <g><text x="160" y={detailed || compactOutcome ? (compact ? "28" : "28") : "37"} textAnchor="middle" fill="#f8fafc" fontSize={compact ? compactOutcome ? "7" : "9" : "11"} fontWeight="700">{play.semanticLabel ?? "GAME EVENT"}</text>{play.semanticSecondaryLabel && (!compact || compactOutcome) ? <text x="160" y={compact ? "40" : "40"} textAnchor="middle" fill="#fde68a" fontSize="7" fontWeight="700">{play.semanticSecondaryLabel}</text> : null}{!compact && play.semanticContextLabel ? <text x="160" y={play.semanticSecondaryLabel ? "50" : "42"} textAnchor="middle" fill="#d1fae5" fontSize="6" fontWeight="700">{play.semanticContextLabel}</text> : null}</g>;
  }
  if (play.start === null) return null;
  const attackingLeft = direction === "left";
  const start = play.animation === "sack" ? attackingLeft ? Math.min(100, play.start + 4) : Math.max(0, play.start - 4) : play.start;
  const final = play.end ?? play.start;
  const marker = play.animation === "incomplete" ? start + (attackingLeft ? -2 : 2) : start + (final - start) * progress;
  ...
  if (play.animation === "field-goal") { const kickStart = x(attackingLeft ? Math.min(100, start + 7) : Math.max(0, start - 7)), kickEnd = attackingLeft ? 13 : 307, t = progress, ballX = (1-t)*(1-t)*kickStart + 2*(1-t)*t*((kickStart+kickEnd)/2) + t*t*kickEnd, ballY = (1-t)*(1-t)*35 + 2*(1-t)*t*7 + t*t*20; return <g><path d={`M ${kickStart} 35 Q ${(kickStart + kickEnd) / 2} 7 ${kickEnd} 20`} fill="none" stroke="#fbbf24" strokeWidth="1.35" strokeDasharray="3 2" opacity=".7" /><ellipse cx={ballX} cy={ballY} rx="3.7" ry="2.1" fill={play.scoring === "field-goal-missed" ? "#fb7185" : "#fbbf24"} stroke="#78350f" strokeWidth=".5" transform={`rotate(-18 ${ballX} ${ballY})`} />{progress >= 1 ? <text x={attackingLeft ? "18" : "282"} y="53" fill={play.scoring === "field-goal-missed" ? "#fda4af" : "#bbf7d0"} fontSize="7" fontWeight="700">{play.scoring === "field-goal-missed" ? "NO GOOD" : "GOOD"}</text> : null}</g>; }
  if (play.animation === "punt") {
    const { kickEnd, returnEnd } = footballPuntCoordinates(play);
    if (kickEnd === null) return null;
    const hasReturn = returnEnd !== null;
    // A parsed landing spot is the kick endpoint. The structured endpoint is only the return result.
    const split = .66;
    ...
    const kick = footballPuntSvgGeometry(start, kickEnd, kickProgress);
    const ballX = hasReturn && progress > split ? x(kickEnd + (returnEnd! - kickEnd) * returnProgress) : kick.renderedBallX;
    const ballY = hasReturn && progress > split ? 40 : kick.renderedBallY;
    return <g><path d={kick.pathD} fill="none" stroke="#c4b5fd" strokeWidth="1.2" strokeDasharray="3 2" opacity=".75" />{hasReturn ? <path d={`M ${x(kickEnd)} 40 L ${x(returnEnd!)} 40`} fill="none" stroke="#7dd3fc" strokeWidth="1.25" strokeDasharray="2 2" opacity=".85" /> : null}<ellipse cx={ballX} cy={ballY} rx="3.7" ry="2.1" fill={hasReturn && progress > split ? "#bae6fd" : "#e9d5ff"} stroke={hasReturn && progress > split ? "#0c4a6e" : "#4c1d95"} strokeWidth=".55" transform={`rotate(-18 ${ballX} ${ballY})`} /></g>;
  }
...
export default function FootballPlayField({ play, offense, homeTeam, awayTeam, compact = false, replayEnabled = true }: { play: FootballVisualizationPlay | null; offense?: string | null; homeTeam?: FootballFieldTeam; awayTeam?: FootballFieldTeam; compact?: boolean; replayEnabled?: boolean }) {
  ...
  const direction = footballTeamAttackDirection(play?.offenseTeamId, homeTeam?.id, awayTeam?.id);
